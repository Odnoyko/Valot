...
export class ReportsPage {
    ...
    _updateReports() {
        ...
        // Update statistics
        this._updateStatistics(filteredTasks);
    ...
    _prepareChartData(filteredTasks) {
        ...
        if (this.chartFilters.period === 'week') {
            ...
            // Track time by project for each day
            relevantEntries.forEach(entry => {
                if (!entry.start_time) return;

                let dateString = entry.start_time;
                if (!dateString.endsWith('Z') && !dateString.includes('+')) {
                    dateString = dateString + 'Z';
                }

                const entryDate = GLib.DateTime.new_from_iso8601(dateString, null);
                if (!entryDate) return;

                const key = `${entryDate.get_year()}-${String(entryDate.get_month()).padStart(2, '0')}-${String(entryDate.get_day_of_month()).padStart(2, '0')}`;
            ...
            // Add time entries to corresponding weeks
            relevantEntries.forEach(entry => {
                if (!entry.start_time) return;
                ...
                const entryJsDate = new Date(entryDate.get_year(), entryDate.get_month() - 1, entryDate.get_day_of_month());
            ...
            // Add time entries to corresponding months
            relevantEntries.forEach(entry => {
                if (!entry.start_time) return;
                ...
                const key = `${entryDate.get_year()}-${String(entryDate.get_month()).padStart(2, '0')}`;
    ...
    _prepareCustomByWeeks(entries, filteredTasks, fromDate, toDate) {
        ...
        entries.forEach(entry => {
            ...
            const weekNum = this._getWeekNumber(entryDate);
            const key = `KW${weekNum}`;
    ...
    _updateStatistics(tasks) {
        ...
        // Active Projects
        const activeProjectIds = new Set(tasks.map(t => t.project_id).filter(id => id));
        this.activeProjectsLabel.set_label(activeProjectIds.size.toString());

        // Tracked Tasks
        this.trackedTasksLabel.set_label(tasks.length.toString());

        // Total Earnings by currency
        const currencyTotals = new Map();

        tasks.forEach(task => {
            if (task.client_rate && task.total_time) {
                const hours = task.total_time / 3600;
                const earnings = hours * task.client_rate;
                const currency = task.client_currency || 'USD';

...
import { DateRange, DateFilters } from '../utils/DateFilters';
 ...
 */
export type ReportGroupBy = 'project' | 'client' | 'task' | 'date' | 'none';

/**
 * Report sort options
 */
export type ReportSortBy = 'duration' | 'date' | 'name' | 'earnings';
...
export class ReportService extends BaseService {
     ...
     */
    private async getEntriesInRange(options: ReportOptions): Promise<TimeEntry[]> {
        // Get all time entries
        const allEntries = await this.core.services.tracking.getAllTimeEntries();

        // Filter by date range
        let filtered = allEntries.filter((entry: TimeEntry) => {
     ...
     */
    private async fetchRelatedData(entries: TimeEntry[]): Promise<{
        projects: Map<string, Project>;
        clients: Map<string, Client>;
        tasks: Map<string, Task>;
        instances: Map<string, TaskInstance>;
     ...
     */
    private calculateSummary(entries: TimeEntry[], dateRange: DateRange): ReportSummary {
     ...
     */
    generateChartData(reportData: ReportData): ChartDataPoint[] {

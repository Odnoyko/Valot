...
export class InputValidator {
    ...
    static validateProjectName(name) {
        ...
        if (trimmed.length > this.MAX_NAME_LENGTH) {
            return {
                valid: false,
                error: `Project name too long (max ${this.MAX_NAME_LENGTH} characters)`,
        ...
        if (this.DANGEROUS_CHARS.test(trimmed)) {
            return {
                ...
                sanitized: trimmed.replace(this.DANGEROUS_CHARS, '')
            };
        }
        ...
        if (this.SQL_INJECTION_PATTERNS.test(trimmed)) {
            return {
                valid: false,
                error: 'Project name contains potentially dangerous content',
    ...
    static validateTaskName(name) {
        ...
        if (trimmed.length > this.MAX_NAME_LENGTH) {
            return {
                valid: false,
                error: `Task name too long (max ${this.MAX_NAME_LENGTH} characters)`,
                sanitized: trimmed.substring(0, this.MAX_NAME_LENGTH)
            };
        }
        ...
        if (this.SQL_INJECTION_PATTERNS.test(trimmed)) {
            return {
                valid: false,
                error: 'Task name contains potentially dangerous content',
                sanitized: trimmed.replace(this.SQL_INJECTION_PATTERNS, '')
            };
        }
    ...
    static validateClientName(name) {
        ...
        if (trimmed.length > this.MAX_NAME_LENGTH) {
            return {
                valid: false,
                error: `Client name too long (max ${this.MAX_NAME_LENGTH} characters)`,
                sanitized: trimmed.substring(0, this.MAX_NAME_LENGTH)
            };
        }
        ...
        if (this.SQL_INJECTION_PATTERNS.test(trimmed)) {
            return {
                valid: false,
                error: 'Client name contains potentially dangerous content',
                sanitized: trimmed.replace(this.SQL_INJECTION_PATTERNS, '')
            };
        }
